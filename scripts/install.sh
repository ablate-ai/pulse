...
PULSE_SERVER_NODE_CLIENT_CERT_FILE server 访问节点时使用的客户端证书路径
  ...
  PULSE_STRIPE_SECRET_KEY     Stripe Secret Key（sk_live_xxx，启用商店必填）
  PULSE_STRIPE_WEBHOOK_SECRET Stripe Webhook Signing Secret（whsec_xxx）
  PULSE_SHOP_ENABLED          设为 true 启用商店页面（需同时配置 STRIPE_SECRET_KEY）
...
if [ "$component" = "server" ]; then
  ...
  if [ "${PULSE_SHOP_ENABLED+x}" = "x" ]; then
    set_env_file_value "$env_target" "PULSE_SHOP_ENABLED" "$PULSE_SHOP_ENABLED"
  fi
