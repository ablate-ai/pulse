...
- [x] 订阅访问日志（记录 /sub/:token 访问时间 + IP + UA，面板可查）
- [ ] 支持二维码生成
- [ ] 支持用户模板应用
...
- [x] 支持基础通知机制（Bark 告警：节点离线 / sing-box 停止 / 用户超限 / 用户到期）
- [x] sing-box 停止时自动恢复下发配置
- [x] 用户-节点流量分布分析（user_node_daily_usage，面板可查）
- [ ] 支持基础运营报表
...
- [x] 完成登录页
- [x] 完成用户管理页（列表 / 新建 / 编辑 / 流量展示 / 订阅日志 / 节点用量分布）
...
- [ ] 完成系统配置页（核心参数编辑）
- [x] 完成订阅链接复制（用户列表"订阅"按钮）
- [x] 完成 Monitor 状态监控页（节点实时指标 SSE / 版本 / 连接数 / 速率）
- [x] 完成 Settings 告警配置（Bark URL 保存 / 测试推送）
- [ ] 完成订阅二维码生成
...
- [x] Bark 推送告警（节点离线 / sing-box 停止 / 超限 / 到期，30 分钟去重）
- [ ] Webhook 通知
- [ ] Telegram Bot
- [ ] 管理 CLI
- [ ] 数据导入导出与备份恢复
